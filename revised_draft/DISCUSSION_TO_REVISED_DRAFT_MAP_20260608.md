...
9. **Width/lift interpretation repaired.** A post-check found that the relative persona-lift ratio shares the unconditioned RBO denominator used in the prompt-width heuristic. The draft now reports absolute unconditioned and same-persona RBO beside relative lift and treats lift as baseline-normalised descriptive behaviour that must be read with its absolute denominator context.
10. **Persona recoverability and leakage bounded.** The draft now adds a within-model leave-one-question-out persona classifier, reruns that classifier after profile-proximate stripping, and explicitly reports profile-proximate language leakage on the external prompt bank. Persona conditioning is therefore described as a bounded salience shift: profile-proximate language contributes to the signal, but does not exhaust the within-model persona effect.
...
- **Vocabulary leakage.** Persona conditioning is treated as an output-level salience intervention, with leakage, stripping, and within-model recoverability analyses supplied in the supporting materials:
  ...
  - `external_infinity_chat_100/parity_analysis/STRIPPED_WITHIN_MODEL_PERSONA_RECOVERABILITY.md`
- **Width/lift denominator coupling.** The draft no longer lets relative persona lift stand alone. Because relative lift divides by unconditioned RBO and the width score also uses unconditioned RBO, the paper now reports absolute unconditioned RBO, absolute same-persona RBO, and relative lift together.
...
- **Persona/model tension.** The apparent tension is now the finding: persona prompts shift surfaced content priorities, but the full output signature remains dominated by base model. A within-model leave-one-question-out classifier confirms that persona signal is detectable once model identity is held fixed, while remaining much weaker than model identity; rerunning this classifier after profile-proximate stripping shows that profile-adjacent vocabulary contributes to the signal but does not exhaust it.
...
| `external_infinity_chat_100/parity_analysis/STRIPPED_WITHIN_MODEL_PERSONA_RECOVERABILITY.md` | Leakage-stripped within-model persona recoverability | Persona recoverability weakens after stripping profile-proximate concepts but remains above five-way chance. |
| `external_infinity_chat_100/parity_analysis/WIDTH_LIFT_DENOMINATOR_AUDIT_20260609.md` | Prompt-width and relative-lift denominator audit | Why relative lift is now interpreted as baseline-normalised behaviour that must be read with absolute RBO context. |
